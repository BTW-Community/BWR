...
public class BWREngineBreedAnimal {
	...
	// using a 3D flood-fill algorithm like block lighting. It adds up the
	// quantities
	...
	public boolean tryBreed(EntityAnimal self) {
		...
		if ((mates != null) && (mates.size() > 0))
			for (int i = 0; i < mates.size(); i++) {
				...
				if (world.getBlockLightValue(MathHelper.floor_double(ent.posX), MathHelper.floor_double(ent.posY),
						MathHelper.floor_double(ent.posZ)) > 4)
					continue;
		...
		if (world.getBlockMaterial(cbx, cby, cbz) == Material.water)
			addProb(weights, eidSquid,
					habitat[Block.waterMoving.blockID * 16] + habitat[Block.waterStill.blockID * 16]);
		...
		addProb(weights, eidVillager,
				(habitat[Block.blockEmerald.blockID] / 100) + (habitat[Block.blockDiamond.blockID] / 30));
		...
		for (Map.Entry<Integer, Integer> p : weights.entrySet()) {
			pick -= p.getValue();
			if (pick <= 0) {
				child = (EntityLiving) EntityList.createEntityByID(p.getKey().intValue(), world);
				break;
			}
		}
		...
		// Play breeding sounds for both parents. Normally we only play one, but since
		// the parents
		// are different species, they'll have different sounds which both need
		// representation.
